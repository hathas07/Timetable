...
public class TimeTableController implements ITimeTableController{
	...
	public TimeTableController(String tTfile) {
		...
		//On charge la DB ‡ la crÈation du controler 
		this.loadDB();
	}
	...
	public boolean addRoom(int roomId, int capacity) {
		...
		try {
			...
			//On regarde si la roomId existe dÈj‡ dans la DB
			for(int roomid: this.tTDB.getRoomDB().keySet()) {
				if(roomid == roomId) {
					throw new Exception("Impossible d'inserer l'emploi du temps : "+roomId+" existe dÈj‡");
				}
			}
	...
	public boolean removeRoom(int roomId) {
		boolean result = true;
		try {
			//Si l'id n'existe pas dans la hashtable, alors elle restera inchangÈ donc pas besoin de chercher si elle existe
			this.tTDB.RemoveRoom(roomId);
			
			//On supprime toute les rÈservations qui avaient lieu dans cette room :
	...
	public boolean addTimeTable(int timeTableId) {
		...
		try {
			...
			//On regarde si la timeTableId existe dÈj‡ dans la DB
			for(int timetableid: this.tTDB.getTimeDB().keySet()) {
				if(timetableid == timeTableId) {
					throw new Exception("Impossible d'inserer l'emploi du temps : "+timeTableId+" existe dÈj‡");
				}
			}
	...
	public boolean addBooking(int timeTableId, int bookingId, String login, Date dateBegin, Date dateEnd, int roomId) {
		...
		try {
			...
			for(Booking booking: this.tTDB.GetTimeTable(timeTableId).getBookingDB().values()) {
				//On cherche si deux rÈsevations se chechauvent pour un groupe
				...
				//On regarde si la bookingId existe dÈj‡ dans la DB
				if(booking.getBookingId() == bookingId) {
					throw new Exception("Impossible d'inserer la reservation : "+bookingId+" existe dÈj‡");
				}
			}
